...
def do_simplex(model_type=Model_Type.ORIGINAL, dataset=Dataset.MNIST, cv=0, corpus_size=100, test_size=10, decompostion_size=100, test_id=0, r_2_tests=True, decomposition_tests=True):
    """ #TODO: aktualisieren
    ...
    if dataset is Dataset.MNIST:
        classifier, corpus, test_set = c.train_or_load_mnist(RANDOM_SEED, cv, corpus_size, test_size)
...
def run_all_experiments(corpus_size=100, test_size=10, decomposition_size=3, cv=0, 
    test_id=0): 
    ...
    file = Path(file_path)
    mode = "a" if file.is_file() else "w"   # append if file exists
...
if __name__ == "__main__":
    ...
    run_all_experiments(corpus_size=100, test_size=3, decomposition_size=3)
