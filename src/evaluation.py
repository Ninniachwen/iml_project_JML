...
def r_2_scores(classifier, latent_rep_approx, latent_rep_true, weights, test_id, test_data, corpus_data):
...
def create_decompositions(test_data, test_targets, corpus_data, corpus_targets, decompostion_size, weights):
    ...
    for s_id, sample, target in zip(range(len(test_data)), test_data, test_targets):
        ...
        per_sample["img"] = sample.reshape([sample.shape[1],sample.shape[2]])
        per_sample["target"] = target.item()
        per_sample["decomposition"] = decompostion
    ...
    {'id': range(len(test_data)),
     'data': list(test_data),
     'target': test_targets.tolist()
    })
    test_data_i = zip(range(len(test_data)), test_data, test_targets)
    test_data_i = zip(range(len(corpus_data)), corpus_data, corpus_targets)
