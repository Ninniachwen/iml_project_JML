...
def do_simplex(model_type, corpus_inputs, corpus_latents, test_inputs, test_latents, classifier, cv, reg_factor_init, reg_factor_final, n_epoch, decompostion_size, test_id):
    ...
    if model_type is Model_Type.ORIGINAL:
        ...
        m.original_model(n_epoch, corpus_inputs, corpus_latents, test_inputs, test_latents, decompostion_size)


    if model_type is Model_Type.ORIGINAL_COMPACT:
        print(f"Starting on cv {cv} with the reimplemented model!")
        m.compact_original_model(n_epoch, corpus_inputs, corpus_latents, test_inputs, test_latents, decompostion_size)
        
    if model_type is Model_Type.REIMPLEMENTED:
        print(f"Starting on cv {cv} with our own model!")
        m.reimplemented_model(n_epoch, corpus_inputs, corpus_latents, test_inputs, test_latents, decompostion_size)
    ...
    fig1 = plot_mnist(test_inputs[test_id][0], f"Test example {test_id}")
...
# 1. r2 repr (jasmin)
# 2. r2 score zahl (jasmin)
# 3. welche bilder sind die top x erklärungen, zahl, id, weights
...
def do_mnist_experiment(cv=0):
    ...
    corpus_loader = load_mnist(100, train=True, shuffle=False)
    test_loader = load_mnist(10, train=False, shuffle=False)
